...
import type { AgentDecisionRecord } from "../types/agent";

/**
 * Persist an agent decision to the database.
 */
export async function persistAgentDecision(
  db: D1Database,
  ...
  await db
    .prepare(
      `INSERT INTO agent_decisions (
        chat_id, message_id, action, content, reasoning,
        confidence, resolution_signal, tools_used, execution_time_ms, is_retry
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .bind(
      decision.chatId,
      ...
      decision.reasoning,
      decision.confidence,
      decision.resolutionSignal,
      JSON.stringify(decision.toolsUsed),
      decision.executionTimeMs,
      decision.isRetry ? 1 : 0
    )
    .run();

  const row = await db
    .prepare(
      `SELECT id FROM agent_decisions WHERE chat_id = ? ORDER BY created_at DESC LIMIT 1`
    )
    .bind(decision.chatId)
    .first<{ id: number }>();
...
/**
 * Update conversation state after agent action.
 */
export async function updateAgentState(
  ...
  if (updates.agentDraftPending !== undefined) {
    setClauses.push("agent_draft_pending = ?");
    values.push(updates.agentDraftPending ? 1 : 0);
  }

  if (updates.lastDraftSentAt !== undefined) {
    setClauses.push("last_draft_sent_at = ?");
    values.push(updates.lastDraftSentAt);
  }

  if (updates.lastDraftId !== undefined) {
    setClauses.push("last_draft_id = ?");
    values.push(updates.lastDraftId);
  }

  if (updates.resolutionStatus !== undefined) {
    setClauses.push("resolution_status = ?");
    values.push(updates.resolutionStatus);
  }

  if (updates.solutionAttemptCount !== undefined) {
    setClauses.push("solution_attempt_count = ?");
    values.push(updates.solutionAttemptCount);
  }

  if (updates.threadConfidenceScore !== undefined) {
    setClauses.push("thread_confidence_score = ?");
    values.push(updates.threadConfidenceScore);
  }
  ...
  await db
    .prepare(
      `UPDATE conversation_state SET ${setClauses.join(", ")} WHERE chat_id = ?`
    )
    .bind(...values)
    .run();
}
